...
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/LoansRegisterCEEP.rdlc';

    dataset
    {
        dataitem(Loans; "Loans Register")
        {
            DataItemTableView = sorting("Staff No") order(ascending) where(Source = filter(MICRO));
            RequestFilterFields = Source, "Loan Product Type", "Date filter", "Application Date", "Loan Status", "Issued Date", Posted, "Batch No.", "Captured By", "Branch Code", "Outstanding Balance", "Loan  No.", "Loan Officer";
